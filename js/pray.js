        ...
        if (!area) {
            ...
            fetch(`https://api.waktusolat.app/v2/solat/${area}`)
                ...
                .then(data => {
                    ...
                    var maghrib = new Date(prayerTimes.maghrib * 1000).toLocaleTimeString();
                ...
                .catch(error => {
                    ...
                    document.getElementById('prayerTimes').textContent = 'Error fetching prayer times.';
                });
        }
    } catch (error) {
        console.error('Error fetching data:', error);
...
function getZone(city) {
    ...
    // Sabah
        ...
        zone = "SBH02";
    } else if (["Lahad Datu", "Silabukan", "Kunak", "Tungku", "Bahagian Tawau (Timur)"].includes(city)) {
        zone = "SBH03";
    } else if (["Tawau", "Balong", "Merotai", "Kalabakan", "Bahagian Tawau (Barat)"].includes(city)) {
        zone = "SBH04";
        ...
        zone = "SBH07";
    } else if (["Papar", "Ranau", "Kota Belud", "Tuaran", "Penampang", "Kota Kinabalu", "Putatan", "Bahagian Pantai Barat"].includes(city)) {
        zone = "SBH08";
    } else if (["Pensiangan", "Keningau", "Tenom", "Nabawan", "Sook"].includes(city)) {
        zone = "SBH09";
    } else if (["Sipitang", "Long Pa Sia", "Beaufort", "Kuala Penyu", "Weston", "Menumbok", "Sindumin", "Membakut"].includes(city)) {
        zone = "SBH10";
    }
    ...
    // Sarawak
        ...
        zone = "SWK07";
    } else if (["Sri Aman", "Lubok Antu", "Engkilili", "Betong", "Saratok", "Roban", "Debak", "Spaoh", "Pusa", "Maludam", "Kabong"].includes(city)) {
        zone = "SWK08";
    } else if (["Kuching", "Bau", "Lundu", "Samarahan", "Simunjan", "Serian", "Sebuyau", "Meludam"].includes(city)) {
        zone = "SWK09";
    }
    ...
    // Selangor
        ...
        zone = "SGR01";
    } else if (["Klang", "Kuala Langat", "Kuala Selangor", "Sabak Bernam"].includes(city)) {
        zone = "SGR02";
    ...
    // Terengganu
        ...
        zone = "TRG04";
    } else if (["Pulau Perhentian", "Pulau Redang", "Pulau Lang Tengah", "Pulau Kapas", "Pulau Tenggol"].includes(city)) {
        zone = "TRG05";
    }
